...
//xu ly db
				...
				System.out.print("update: "+message);
				...
				if(data.getAct().equalsIgnoreCase("SET")){
					...
					if(data.getAct().equalsIgnoreCase("DEL")){
...
//xu ly db
	...
	{	Hashtable hash = new Hashtable();
		...
		int currentCircle=0;int loop=1;
		try{
		
		GetState gs = new GetState("Server1");
		gs.getCurrentCircle();
		gs.sendUpdate("116.103.17.3",2002,"Server1");
								
		ServerSocket server = new ServerSocket(2001);
		while(true)
		{				
				int localPort = server.getLocalPort();
				display.append("Server 1 is listening on port "+localPort+".\n");
				Socket client = server.accept();
				app.handler(client,"Server1",1,currentCircle,hash);
				app.runServer();
